...
void MiniKame::init(){
   ...
   // ok CARLOS: original pinout from Sergei
   // but all is narrow 
    /*board_pins[0] = D1; //D1 front left inner  S0
    board_pins[1] = D4, //D4 front right inner S1
    board_pins[2] = D2; //D8 front left outer  S2
    board_pins[3] = D3; //D6 front right outer S3
    board_pins[4] = D7; //D7 back left inner   S4
    board_pins[5] = D5; //D5 back right inner  S5
    board_pins[6] = D8; //D2 back left outer   S6
    board_pins[7] = D6; //D3 back right outer  S7*/

// carlos: pinout like javierL-Kame original
    board_pins[0] = D1; //D1 front left inner  S0
    board_pins[1] = D4, //D4 front right inner S1
    board_pins[2] = D8; //D8 front left outer  S2
    board_pins[3] = D6; //D6 front right outer S3
    board_pins[4] = D7; //D7 back left inner   S4
    board_pins[5] = D5; //D5 back right inner  S5
    board_pins[6] = D2; //D2 back left outer   S6
    board_pins[7] = D3; //D3 back right outer  S7
    // inner: 0, 1, 4, 5
...
// CARLOS: this secction allow reverse servos
